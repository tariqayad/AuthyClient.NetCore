...
namespace AuthyClient
{
    ...
    public class AuthyApiClient : IAuthyApiClient
    {
        ...
        public string CreateAuthyUser(string email, string cellphone, int countryCode = 1)
        {
            ...
            return CreateAuthyUser(new AuthyUser(email, cellphone, countryCode)).Result;
        }
        ...
        /// <returns>Returns an Authy User Id to store. This is necessary for authenticating a user's token later</returns>
        /// <exception cref="AuthyClientException">Thrown when the Authy Api response is not HttpStatusCode 2xx</exception>
        public async Task<string> CreateAuthyUser(AuthyUser request)
        {
            ...
            var response = await client.ExecuteTaskAsync(authyRequest);
            ...
            throw new AuthyClientException(response.Content, new Dictionary<string, string>());

            //client.ExecuteAsync(authyRequest, (res) =>
            //{
            //    if ((int)res.StatusCode >= (int)HttpStatusCode.OK && (int)res.StatusCode < 300)
            //    {
            //        returnVal = res.Content;
            //        tcs.SetResult(returnVal);
            //    }
            //    else
            //    {
            //        var errors = new Dictionary<string, string>();
            //        errors.Add("error", res.ErrorMessage);
            //        throw new AuthyClientException(res.Content, errors);
            //    }
            //});


            //return returnVal;
            //var tcs = new TaskCompletionSource<IRestResponse<string>();
            //var response =  await client.ExecuteAsync<IRestResponse<string>>(authyRequest, r =>
            //{
            //    if ((int)r.StatusCode >= (int)HttpStatusCode.OK && (int)r.StatusCode < 300)
            //    {
        ...
        public async Task<bool> VerifyUserToken(string authyUserId, string authyToken, bool force = true)
        {
            ...
            bool retValue = false;
            var resp = await client.ExecuteTaskAsync(authyRequest);
        ...
        public async void SendToken(string authyUserId, bool forceSmsSend = false)
        {
            if (!NumberIsValid(authyUserId))
